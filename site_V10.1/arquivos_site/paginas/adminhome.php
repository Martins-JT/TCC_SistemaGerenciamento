...
p.marca, p.unidade_medida, o.exibir_vitrine, o.carrinho, o.identificador
from ofertas o
...
<div class="area1" id="area"> <!-- Abertura da div area(primeira área da home) -->
      ...
      <div class="controle-icon">
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" 
      viewBox="0 0 24 24">
      <path fill="currentColor" d="M9.5 3A6.5 6.5 0 0 1 16 9.5c0 1.61-.59 3.09-1.56 4.23l.27.27h.79l5 5l-1.5 1.5l-5-5v-.79l-.27-.27A6.52 6.52 0 0 1 9.5 16A6.5 6.5 0 0 1 3 9.5A6.5 6.5 0 0 1 9.5 3m0 2C7 5 5 7 5 9.5S7 14 9.5 14S14 12 14 9.5S12 5 9.5 5"/></svg>
      </div>
      <input type="text" name="" id="selecionarNome" oninput="pesquisa()" placeholder="Pesquisar Empresa">
      </div>
      <p><span class="identificar2">Vitrine Virtual</span></p>
      <div class="container_vitrine"> <!-- Abertura div container_vitrine -->
    ...
    if ($linha['exibir_vitrine'] && !$linha['carrinho'] && !$linha['identificador']){
...
?>
  ...
  <div class="ofertas testar" data-nome_empresa="<?php echo $linha['nome_empresa']; ?>" data-id="<?php echo $linha['id_oferta']; ?>"><!-- ABERTURA DIV OFERTAS -->
  ...
  <div class="controle_conteudo" id="detalhes-conteudo-<?php echo $linha['id_oferta']; ?>" onclick="fecharaoclicarfora(event, '<?php echo $linha['id_oferta']; ?>')"><!-- POSICIONAMENTO/CONFIGURAÇÃO DO CONTEÚDO (ABERTURA DIV CONTROLE_CONTEUDO)-->
    <div class="conteudo"><!-- ABERTURA DIV DO CONTEÚDO INTERNO -->
      ...
      <div class="produtos"> <!-- ABERTURA DIV PRODUTOS -->
        ...
        <input type="hidden" name="id_vitrine" value="<?php echo $linha['id_oferta']; ?>">
...
?>
      </div><!-- Fechamento div container_vitrine -->
...
<div class="area1" id="area2"> <!-- Abertura da div area2(segunda área da home) -->
  <p><span class="identificar2">Ofertas</span></p>
    <div class="container_oferta"> <!-- Abertura div container_oferta -->
    <?php
    foreach ($produtos as $linha){// Usar $result aqui, não $resultado
    if ($linha['status'] && !$linha['exibir_vitrine'] && !$linha['carrinho'] && !$linha['identificador']){
    $dataInicio = $linha['data_inicio'] ?? null;
    $diasSelecionados = $linha['dias'] ?? null;
...
?>
    ...
    </div><!-- Fechamento div container_oferta -->
</div><!-- Fechamento div area2(segunda) -->


<div class="area1" id="area3"> <!-- Abertura da div area3(terceira área da home) -->
  <p><span class="identificar2">Carrinho</span></p>
    <div class="container_carrinho"> <!-- Abertura div container_carrinho -->
     
     <?php
    foreach ($produtos as $linha){// Usar $result aqui, não $resultado
    if ($linha['carrinho'] && !$linha['identificador']){
    $dataInicio = $linha['data_inicio'] ?? null;
    $diasSelecionados = $linha['dias'] ?? null;
...
?>
  ...
  <div class="controle_conteudo" id="detalhes-conteudo-<?php echo $linha['id_oferta']; ?>" onclick="fecharaoclicarfora(event, '<?php echo $linha['id_oferta']; ?>')"><!-- POSICIONAMENTO/CONFIGURAÇÃO DO CONTEÚDO (ABERTURA DIV CONTROLE_CONTEUDO)-->
    ...
    </div><!-- Fechamento div container_carrinho -->
    <div class="valor_car">
...
<script>
  ...
  function pesquisa()
{
  let nomeselecionado = document.getElementById('selecionarNome').value.toLowerCase();
  let nomeEmpresa = document.getElementsByClassName('ofertas');

  for(let i = 0; i < nomeEmpresa.length; i++) /* .LENGTH (RECONHECE O TAMANHO DE UMA STRING OU ARRAY) */
  {
    
    let elemento = nomeEmpresa[i].getAttribute('data-nome_empresa').toLowerCase();
    if (elemento.includes(nomeselecionado)){
    nomeEmpresa[i].style.display = 'block';
    }
    else{
    nomeEmpresa[i].style.display = 'none';
    }
  }
}

  function Teste() {
    let nomeSelecionado = document.getElementById('nomeselecionado').value;
    let apelidos = document.getElementsByClassName('testar');
    
    for (let i = 0; i < apelidos.length; i++) {
        let elemento = apelidos[i];
        
        let nome = elemento.getAttribute('data-nome_empresa');
       
        console.log(nome);
        if (nomeSelecionado == "" || nome === nomeSelecionado) {
...
function removerOferta(idOferta) {
  const oferta = document.querySelector('.ofertas[data-id="' + idOferta + '"]');

  if (oferta) {
    oferta.remove();
    //console.log('Oferta removida do DOM:', idOferta);
  } /* else {
    console.warn('Oferta não encontrada com data-id:', idOferta);
  } */
}
...
function excluirOferta(idOferta) {
  ...
  xhr.open('POST', '../../arquivos_php/remover_oferta.php', true);
  xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
  xhr.onload = function () {
    if (xhr.status === 200) {
      removerOferta(idOferta);
    } else {
      console.error('Erro ao remover a oferta no servidor.');
    }
  };
  xhr.send('id=' + encodeURIComponent(idOferta));
}
